...
const sendNotification = async (req, res) => {
  ...
  const public_userId = req.params.public_userId
  const shopId = req.params.shopId
  const offerId = req.params.offerId
  const db = new getDatabase()

  if (!shopId) {
    res.status(401).send({ message: 'No shop Id found' })
    return
  }
  ...
  const userPurchases = shop.offer.user_purchases
  const maxPurchases = shop.offer.max_purchases
  let isCompleted = false

  if (userPurchases === maxPurchases) {
    isCompleted = true
  }
  ...
  const options = {
    ...
    body: JSON.stringify({
      ...
      name: 'Test-push',
      include_player_ids: [playerId], // Canviar per playerId
      contents: { en: 'Approximitat', es: 'Approximitat' },
      headings: {
        en: !isCompleted ? `${shopName} acaba de validar tu compra` : `Has completat l'oferta de ${shopName}.`,
        es: !isCompleted ? `${shopName} acaba de validar tu compra` : `Has completat l'oferta de ${shopName}.`,
      }
    })
  };
  ...
  try {
    ...
    await db.saveNotification(user.id, shopId, offerId)
  }
